## Welcome to my humble abode! 🦁

I'm Imani Rojas 👋🏼

I'm a Graphic Information Technology student with an interest in front-end web development, graphic design, and creating engaging digital experiences. 

As I continue building my skills in coding and design, I'm exploring how thoughtful user experiences, visual storytelling, and interactive interfaces can come together to solve real-world problems.

## 📫 Contact Information

- Email: imanirojas02@gmail.com
- LinkedIn: https://www.linkedin.com/in/imani-rojas/
- GitHub: https://github.com/mani-R2

## 💻 Coding Languages & Technologies

- HTML5
- CSS3
- JavaScript
- Git & GitHub
- Responsive Web Design
- Visual Studio Code
- Figma
- Adobe Creative Cloud (Illustrator, Photoshop, InDesign)

## 🚀 Current Project

- **Mood-Based Music Recommender** – A front-end web application that helps users discover playlists based on their current mood using color themes, animations, and an intuitive user experience.


## 🌱 Currently Learning

- Advanced JavaScript
- Front-end development best practices
- User experience (UX) design
- Accessibility and responsive design
- CSS animations and interactions

## 🎨 Passion Areas

- Front-end web development
- UI/UX design
- Graphic design
- Interactive experiences
- Visual storytelling

## 🔗 Featured Projects

- **Personal Portfolio Website** – A responsive portfolio showcasing my design and development projects.
- **GitHub Profile Development** – Building and documenting projects while strengthening my coding skills.

Check out my repositories to see my latest work and progress as I continue growing my skills in web development and design.

Thanks for stopping by!

![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=mani-R2&show_icons=trueprs&cache_seconds=86400&theme=rose)
